...
# Dan voorspellingen maken op basis van de waarden (je hebt liever veel voorspellingen om een mooie gecurvde lijn te tekenen)
bbp_preds <- predictions(multiple_ln, 
                         newdata = datagrid(LNGDP2006= seq(from=5.5, to=11, by=0.5)))

#exponentiëren van gelogde predictor
bbp_preds <- bbp_preds |> 
  mutate(GDP2006 = exp(LNGDP2006))
...
-   Is de kwadratische predictor negatief, dan is de relatie tussen X en Y concaaf (omgekeerde U)

-   Is de kwadratische predictor positief, dan is de relatie tussen X en Y convex (U-vorm)

Wederom is het vaak beter om de relatie visueel te verduidelijken:

```{r}
#Eerst de waarden van X nagaan voor de gebruikte observaties
