...
Het `marginaleffects`-package is niet volledig compatibel met de `factorize` functie. Dit levert vooral problemen op met 'lege categorieën'. Dit zijn categorieën waar wel een label voor is gemaakt in de dataset, maar waar geen respondenten in zitten. Factorvariabelen in de regressie met lege categorieën leiden tot een foutmelding bij gebruik van `predictions`.

Hieronder kijken we bijvoorbeeld naar de gender variabele in de Nederlandse survey voor ronde 9 van de European Social Survey (ESS). Er zijn 3 waardenlabels aangemaakt: "Male", "Female", "No answer". De "No answer" categorie is leeg en zal automatisch wegvallen bij de regressieanalyse. Echter is het label nog aan de variabele bevestigd na `factorize`. Dit leidt tot een foutmelding omdat R op zoek gaat naar een variabele met die categorie. Om dit te vermijden moeten we de overtollige categorie verwijderen uit de dataset met de `droplevels`-functie.
...
Hier gebruiken we `predictions` eerst voor ons binair model.
...
[^linear_05-4]: We zouden technisch gezien wel `newdata = datagrid()` kunnen gebruiken maar dan moeten we de niveaus van de factor variabele manueel typen (bv. `newdata = datagrid(TYPEDEMO1984_factor = c("Autocracies', "Democracies"))`. De `by =` functie is dus gemakkelijker.
...
De dataset die we bekomen met de `by` functie is wel iets anders en we vinden hier de onafhankelijke variabelen niet meer terug.
